...
var formatMealplanAction = (action) => {
...
function isSkipAuth() {
  try {
    return typeof window !== "undefined" && window.location?.search?.includes("skipauth=1");
  } catch {
    return false;
  }
}
...
var assistantFallbackText = "Welcome \u2014 I'm Little Chef.\n\nTo start onboarding, follow the instructions below.";
...
var composeOverlay = null;
var composeActive = false;
var composeDblTapTimer = null;
var composeDblTapCount = 0;
var COMPOSE_DBL_TAP_WINDOW_MS = 350;
var composeNextHintTimer = null;
var composeNextHintShown = false;
var composeNextHintTriggered = false;
...
function renderProposal() {
  ...
  const summaries = state.proposedActions.map((action) => {
    ...
    if (action.action_type === "generate_mealplan" && action.mealplan) {
      ...
      const mealCount = (mp.days ?? []).reduce((s, d) => s + (d.meals?.length ?? 0), 0);
...
async function sendAsk(message, opts) {
  ...
  try {
    ...
    const endpoint = currentFlowKey === "inventory" ? "/chat/inventory" : currentFlowKey === "mealplan" ? "/chat/mealplan" : "/chat";
...
var sentTooltipEl = null;
var sentTooltipTimer = null;
...
function openSettingsModal() {
  if (document.getElementById("lc-settings-modal")) return;
  const overlay = document.createElement("div");
...
function wire() {
  ...
  const auth0CallbackPending = new URLSearchParams(window.location.search).has("code") && new URLSearchParams(window.location.search).has("state");
  refreshSystemHints();
...
function hideComposeOverlay() {
  ...
  const composer = document.getElementById("duet-composer");
  if (input && composer) {
    const micBtn2 = document.getElementById("duet-mic");
    if (micBtn2 && micBtn2.nextSibling) {
      composer.insertBefore(input, micBtn2.nextSibling);
    } else {
